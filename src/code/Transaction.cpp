...
void Transaction::logTransactionToCSV(const std::string& filename) const {
    ...
    file << id << "," << type << "," << cryptoName << "," << quantity << ","
...
std::string Transaction::readTransaction(int i, const std::string& filename) {
    ...
    if (i == 0) {
        ...
        while (getline(file, line)) {}  // Lecture jusqu'à la dernière ligne
        result = line;
    } else if (i > 0) {
        // Lire la ligne spécifiée
        while (getline(file, line)) {
            if (++currentLine == i + 1) {  // Ligne i+1 (car la première ligne est l'en-tête)
                result = line;
                break;
            }
        }
